...
import calendar,datetime,decimal,json,xlwt as xlwt
...
def query(date):
    ...
    data = {"rlist": rlist, "sumlist": sumDict, "erlist": erlist, "esumlist": esumDict, "yearlist": yearlist,
            "yearSum": yearSumDict}
    return data
...
def sum1(slist, days, ddict, mdict, ydict, edict, rlist, sumDict, rlist2, sumDict2, yeardict, yearlist, yearSumDict,
         yydict, yearavgdict, yestoday):
...
def countSum(sumList, days):
    ...
    for key in sumList.keys():
        ...
        if float(sum["day_salevalueesti"]) > 0:
            sum["day_accomratio"] = str(
                "%0.2f" % (float(sum["day_salevalue"]) * 100 / float(sum["day_salevalueesti"]))) + "%"
        else:
            sum["day_accomratio"] = str("0.00%")

        # 日销售-来客数同比增长
        if sum["day_tradenumberold"] > 0:
            sum["day_tradenumber_ynygrowth"] = str("%0.2f" % (
            (sum["day_tradenumber"] - sum["day_tradenumberold"]) * 100.0 / sum["day_tradenumberold"])) + "%"
        else:
        ...
        if sum["day_tradenumberold"] > 0:
            sum["day_tradepriceold"] = str(
                "%0.2f" % (float(sum["day_salevalueold"]) * 10000 / sum["day_tradenumberold"]))
        else:
            sum["day_tradepriceold"] = str("0.00")

        # 日销售-客单价同比增长
        if float(sum["day_tradepriceold"]) > 0:
            sum["day_tradeprice_ynygrowth"] = str("%0.2f" % (
            (float(sum["day_tradeprice"]) - float(sum["day_tradepriceold"])) * 100.0 / float(
                sum["day_tradepriceold"]))) + "%"
        else:
        ...
        if float(sum["day_salegainesti"]) > 0:
            sum["day_salegain_accomratio"] = str(
                "%0.2f" % (float(sum["day_salegain"]) * 100 / float(sum["day_salegainesti"]))) + "%"
        else:
            sum["day_salegain_accomratio"] = str("0.00%")

        # 日销售-毛利毛利率
        if float(sum["day_salevalue"]) > 0:
            sum["day_grossmargin"] = str(
                "%0.2f" % (float(sum["day_salegain"]) * 100 / float(sum["day_salevalue"]))) + "%"
        else:
            sum["day_grossmargin"] = str("0.00%")

        # 月累计-差异
        sum["month_sale_difference"] = str(
            "%0.2f" % (float(sum["month_salevalue"]) - float(sum["month_salevalueesti"])))

        # 月累计-达成率
        if float(sum["month_salevalueesti"]) > 0:
            sum["month_accomratio"] = str(
                "%0.2f" % (float(sum["month_salevalue"]) * 100 / float(sum["month_salevalueesti"]))) + "%"
        else:
            sum["month_accomratio"] = str("0.00%")

        # 月累计-月预算进度
        if float(sum["month_sale_estimate"]) > 0:
            sum["month_complet_progress"] = str(
                "%0.2f" % (float(sum["month_salevalue"]) * 100 / float(sum["month_sale_estimate"]))) + "%"
        else:
            sum["month_complet_progress"] = str("0.00%")

        # 月累计-同比增长
        if float(sum["month_salevalueold"]) > 0:
            sum["month_sale_ynygrowth"] = str("%0.2f" % (
        ...
        # 月毛利-差异
        sum["month_salegain_difference"] = str(
            "%0.2f" % (float(sum["month_salegain"]) - float(sum["month_salegainesti"])))

        # 月毛利-达成率
        if float(sum["month_salegainesti"]) > 0:
            sum["month_salegain_accomratio"] = str(
                "%0.2f" % (float(sum["month_salegain"]) * 100 / float(sum["month_salegainesti"]))) + "%"
        else:
        ...
        if float(sum["month_salevalue"]) > 0:
            sum["month_salegain_grossmargin"] = str(
                "%0.2f" % (float(sum["month_salegain"]) * 100 / float(sum["month_salevalue"]))) + "%"
        else:
            sum["month_salegain_grossmargin"] = str("0.00%")

        # 月毛利-去年同期毛利率
        if float(sum["month_salevalueold"]) > 0:
            sum["month_salegain_grossmarginold"] = str(
                "%0.2f" % (float(sum["month_salegainold"]) * 100 / float(sum["month_salevalueold"]))) + "%"
        else:
            sum["month_salegain_grossmarginold"] = str("0.00%")

        # 月日均来客数-同比增长
        if sum["month_tradenumberold"] > 0:
            sum["month_tradenumber_ynygrowth"] = str("%0.2f" % (
            (sum["month_tradenumber"] - sum["month_tradenumberold"]) * 100.0 / sum["month_tradenumberold"])) + "%"
        else:
        ...
        if sum["month_tradenumber"] > 0:
            sum["month_tradeprice"] = str(
                "%0.2f" % (float(sum["month_salevalue"]) * 10000 / (sum["month_tradenumber"] * days)))
        else:
            sum["month_tradeprice"] = str("0.00")

        # 月客单价-去年月日均客单价=去年月累计销售实际 * 10000 /（去年月累计来客数*天数）
        if sum["month_tradenumberold"] > 0:
            sum["month_tradepriceold"] = str(
                "%0.2f" % (float(sum["month_salevalueold"]) * 10000 / (sum["month_tradenumberold"] * days)))
        else:
            sum["month_tradepriceold"] = str("0.00")

        # 月客单价-同比增长
        if float(sum["month_tradepriceold"]) > 0:
            sum["month_tradeprice_ynygrowth"] = str("%0.2f" % (
            (float(sum["month_tradeprice"]) - float(sum["month_tradepriceold"])) * 100.0 / float(
                sum["month_tradepriceold"]))) + "%"
        else:
...
def setDaiySale(ritem, dayItem):
    ...
    ritem.setdefault('day_sale_difference',
                     mtu.convertToStr(mtu.quantize(dayItem["salevalue"]) - mtu.quantize(dayItem["salevalueesti"]),
                                      "0.00", 1))
    if mtu.quantize(dayItem["salevalueesti"]) > 0:
        ritem.setdefault('day_accomratio', "{day_accomratio}%".format(day_accomratio=mtu.convertToStr(
            mtu.quantize(dayItem["salevalue"]) * decimal.Decimal("100.0") / mtu.quantize(dayItem["salevalueesti"]),
            "0.00", 1)))
    else:
    ...
    if mtu.quantize(dayItem["tradenumberold"], "0", 1) > 0:
        ritem.setdefault('day_tradenumber_ynygrowth', mtu.convertToStr(
            (dayItem["tradenumber"] - dayItem["tradenumberold"]) * decimal.Decimal("100.0") / dayItem["tradenumberold"],
            "0.00", 1) + "%")
    else:
    ...
    if mtu.quantize(dayItem["tradepriceold"], "0.00", 1) > 0:
        ritem.setdefault('day_tradeprice_ynygrowth', mtu.convertToStr(
            (dayItem["tradeprice"] - dayItem["tradepriceold"]) * decimal.Decimal("100.0") / dayItem["tradepriceold"],
            "0.00", 1) + "%")
    else:
    ...
    ritem.setdefault('day_salegain_difference',
                     mtu.convertToStr(mtu.quantize(dayItem["salegain"]) - mtu.quantize(dayItem["salegainesti"]), "0.00",
                                      1))

    if mtu.quantize(dayItem["salegainesti"]) > 0:
        ritem.setdefault('day_salegain_accomratio', mtu.convertToStr(
            mtu.quantize(dayItem["salegain"]) * decimal.Decimal("100.0") / mtu.quantize(dayItem["salegainesti"]),
            "0.00", 1) + "%")
    else:
        ritem.setdefault('day_salegain_accomratio', "0.00")

    if mtu.quantize(dayItem["salevalue"]) > 0:
        ritem.setdefault('day_grossmargin', mtu.convertToStr(
            mtu.quantize(dayItem["salegain"]) * decimal.Decimal("100.0") / mtu.quantize(dayItem["salevalue"]), "0.00",
            1) + "%")
    else:
...
def setValue(sum1, ritem):
    ...
    if "month_salegain_estimate" in sum1:
        sum1["month_salegain_estimate"] = str(
            float(sum1["month_salegain_estimate"]) + float(ritem["month_salegain_estimate"]))
    else:
...
def setMonthSale(ritem, monthItem, yitem):
    ...
    ritem.setdefault('month_sale_difference', mtu.convertToStr(
        mtu.quantize(monthItem["m_salevalue"]) - mtu.quantize(monthItem["m_salevalueesti"]), "0.00", 1))

    if monthItem["m_salevalueesti"] > 0:
        ritem.setdefault('month_accomratio', mtu.convertToStr(
            mtu.quantize(monthItem["m_salevalue"]) * decimal.Decimal("100.0") / mtu.quantize(
                monthItem["m_salevalueesti"]), "0.00", 1) + "%")
    else:
    ...
    if yitem["y_salevalue"] > 0:
        ritem.setdefault('month_complet_progress', mtu.convertToStr(
            mtu.quantize(monthItem["m_salevalue"]) * decimal.Decimal("100.0") / mtu.quantize(yitem["y_salevalue"]),
            "0.00", 1) + "%")
    else:
    ...
    if monthItem["m_salevalueold"] > 0:
        ritem.setdefault('month_sale_ynygrowth', mtu.convertToStr(
            (mtu.quantize(monthItem["m_salevalue"]) - mtu.quantize(monthItem["m_salevalueold"])) * decimal.Decimal(
                "100.0") / mtu.quantize(monthItem["m_salevalueold"]), "0.00", 1) + "%")
    else:
    ...
    ritem.setdefault('month_salegain_difference',
                     mtu.convertToStr(mtu.quantize(monthItem["m_salegain"]) - mtu.quantize(monthItem["m_salegainesti"]),
                                      "0.00", 1))

    if monthItem["m_salegainesti"] > 0:
        ritem.setdefault('month_salegain_accomratio', mtu.convertToStr(
            mtu.quantize(monthItem["m_salegain"]) * decimal.Decimal("100.0") / mtu.quantize(
                monthItem["m_salegainesti"]), "0.00", 1) + "%")
    else:
    ...
    if yitem["y_salegain"] > 0:
        ritem.setdefault('month_salegain_complet_progress', mtu.convertToStr(
            mtu.quantize(monthItem["m_salegain"]) * decimal.Decimal("100.0") / mtu.quantize(yitem["y_salegain"]),
            "0.00", 1) + "%")
    else:
    ...
    if monthItem["m_salegainold"] > 0:
        ritem.setdefault('month_salegain_ynygrowth', mtu.convertToStr(
            (mtu.quantize(monthItem["m_salegain"]) - mtu.quantize(monthItem["m_salegainold"])) * decimal.Decimal(
                "100.0") / mtu.quantize(monthItem["m_salegainold"]), "0.00", 1) + "%")
    else:
        ritem.setdefault('month_salegain_ynygrowth', "0.00")

    if monthItem["m_salevalue"] > 0:
        ritem.setdefault('month_salegain_grossmargin', mtu.convertToStr(
            mtu.quantize(monthItem["m_salegain"]) * decimal.Decimal("100.0") / mtu.quantize(monthItem["m_salevalue"]),
            "0.00", 1) + "%")
    else:
        ritem.setdefault('month_salegain_grossmargin', "0.00")

    if monthItem["m_salevalueold"] > 0:
        ritem.setdefault('month_salegain_grossmarginold', mtu.convertToStr(
            mtu.quantize(monthItem["m_salegainold"]) * decimal.Decimal("100.0") / mtu.quantize(
                monthItem["m_salevalueold"]), "0.00", 1) + "%")
    else:
    ...
    if monthItem["m_tradenumberold"] > 0:
        ritem.setdefault('month_tradenumber_ynygrowth', mtu.convertToStr(
            (monthItem["m_tradenumber"] - monthItem["m_tradenumberold"]) * decimal.Decimal("100.0") / monthItem[
                "m_tradenumberold"], "0.00", 1) + "%")
    else:
    ...
    if monthItem["m_tradepriceold"] > 0:
        ritem.setdefault('month_tradeprice_ynygrowth', mtu.convertToStr(
            (monthItem["m_tradeprice"] - monthItem["m_tradepriceold"]) * decimal.Decimal("100.0") / monthItem[
                "m_tradepriceold"], "0.00", 1) + "%")
    else:
...
def setYearSale(ritem, yearavgdict):
    ...
    ritem.setdefault('sale_difference',
                     mtu.convertToStr(decimal.Decimal(ritem["salevalue"]) - decimal.Decimal(ritem["salevalueesti"]),
                                      "0.00", 1))
    if decimal.Decimal(ritem["salevalueesti"]) > 0:
        ritem.setdefault('sale_accomratio', "{sale_accomratio}%".format(sale_accomratio=mtu.convertToStr(
            decimal.Decimal(ritem["salevalue"]) * decimal.Decimal("100.0") / decimal.Decimal(ritem["salevalueesti"]),
            "0.00", 1)))
    else:
    ...
    if decimal.Decimal(ritem["y_salevalue"]) > 0:
        ritem.setdefault('sale_complet_progress', "{sale_complet_progress}%".format(
            sale_complet_progress=mtu.convertToStr(
                decimal.Decimal(ritem["salevalue"]) * decimal.Decimal("100.0") / decimal.Decimal(ritem["y_salevalue"]),
                "0.00", 1)))
    else:
    ...
    if decimal.Decimal(ritem["salevalueold"]) > 0:
        ritem.setdefault('sale_ynygrowth', mtu.convertToStr(
            (decimal.Decimal(ritem["salevalue"]) - decimal.Decimal(ritem["salevalueold"])) * decimal.Decimal(
                "100.0") / decimal.Decimal(ritem["salevalueold"]), "0.00", 1) + "%")
    else:
    ...
    ritem.setdefault('salegain_difference',
                     mtu.convertToStr(decimal.Decimal(ritem["salegain"]) - decimal.Decimal(ritem["salegainesti"]),
                                      "0.00", 1))
    if decimal.Decimal(ritem["salegainesti"]) > 0:
        ritem.setdefault('salegain_accomratio', "{salegain_accomratio}%".format(salegain_accomratio=mtu.convertToStr(
            decimal.Decimal(ritem["salegain"]) * decimal.Decimal("100.0") / decimal.Decimal(ritem["salegainesti"]),
            "0.00", 1)))
    else:
        ritem.setdefault('salegain_accomratio', "")

    if decimal.Decimal(ritem["y_salegain"]) > 0:
        ritem.setdefault('salegain_complet_progress', "{salegain_complet_progress}%".format(
            salegain_complet_progress=mtu.convertToStr(
                decimal.Decimal(ritem["salegain"]) * decimal.Decimal("100.0") / decimal.Decimal(ritem["y_salegain"]),
                "0.00", 1)))
    else:
    ...
    if decimal.Decimal(ritem["salegainold"]) > 0:
        ritem.setdefault('salegain_ynygrowth', mtu.convertToStr(
            (decimal.Decimal(ritem["salegain"]) - decimal.Decimal(ritem["salegainold"])) * decimal.Decimal(
                "100.0") / decimal.Decimal(ritem["salegainold"]), "0.00", 1) + "%")
    else:
        ritem.setdefault('salegain_ynygrowth', "")

    if decimal.Decimal(ritem["salevalue"]) > 0:
        ritem.setdefault('salegain_grossmargin', "{salegain_grossmargin}%".format(salegain_grossmargin=mtu.convertToStr(
            decimal.Decimal(ritem["salegain"]) * decimal.Decimal("100.0") / decimal.Decimal(ritem["salevalue"]), "0.00",
            1)))
    else:
    ...
    if ritem["tradenumber"] > 0:
        ritem['tradeprice'] = mtu.convertToStr(
            decimal.Decimal(ritem["salevalue"]) * decimal.Decimal("10000") / ritem["tradenumber"], "0.00", 1)
    else:
        ritem['tradeprice'] = "0.00"

    if ritem["tradenumberold"] > 0:
        ritem['tradepriceold'] = mtu.convertToStr(
            decimal.Decimal(ritem["salevalueold"]) * decimal.Decimal("10000") / ritem["tradenumberold"], "0.00", 1)
    ...
    if ritem["tradenumberold"] > 0:
        ritem.setdefault('tradenumber_ynygrowth', mtu.convertToStr(
            (ritem["tradenumber"] - ritem["tradenumberold"]) * decimal.Decimal("100.0") / ritem["tradenumberold"],
            "0.00", 1) + "%")
    else:
        ritem.setdefault('tradenumber_ynygrowth', "")

    if decimal.Decimal(ritem["tradepriceold"]) > 0:
        ritem.setdefault('tradeprice_ynygrowth', mtu.convertToStr(
            (decimal.Decimal(ritem["tradeprice"]) - decimal.Decimal(ritem["tradepriceold"])) * decimal.Decimal(
                "100.0") / decimal.Decimal(ritem["tradepriceold"]), "0.00", 1) + "%")
    else:
...
def countSum3(sumList):
    ...
    for key in sumList.keys():
        ...
        if float(sum["y_salevalue"]) > 0:
            sum["sale_complet_progress"] = str(
                "%0.2f" % (float(sum["salevalue"]) * 100 / float(sum["y_salevalue"]))) + "%"
        else:
            sum["sale_complet_progress"] = str("0.00%")
        # 同比增长
        if float(sum["salevalueold"]) > 0:
            sum["sale_ynygrowth"] = str("%0.2f" % (
            (float(sum["salevalue"]) - float(sum["salevalueold"])) * 100 / float(sum["salevalueold"]))) + "%"
        else:
        ...
        # 日销售-毛利达成率
        if float(sum["salegainesti"]) > 0:
            sum["salegain_accomratio"] = str(
                "%0.2f" % (float(sum["salegain"]) * 100 / float(sum["salegainesti"]))) + "%"
        else:
            sum["salegain_accomratio"] = str("0.00%")
            # 年毛利预算季度
        if float(sum["y_salegain"]) > 0:
            sum["salegain_complet_progress"] = str(
                "%0.2f" % (float(sum["salegain"]) * 100 / float(sum["y_salegain"]))) + "%"
        else:
            sum["salegain_complet_progress"] = str("0.00%")
            # 同比增长
        if float(sum["salegainold"]) > 0:
            sum["salegain_ynygrowth"] = str("%0.2f" % (
            (float(sum["salegain"]) - float(sum["salegainold"])) * 100 / float(sum["salegainold"]))) + "%"
        else:
        ...
        if sum["tradenumberold"] > 0:
            sum["tradenumber_ynygrowth"] = str(
                "%0.2f" % ((sum["tradenumber"] - sum["tradenumberold"]) * 100.0 / sum["tradenumberold"])) + "%"
        else:
        ...
        if float(sum["tradepriceold"]) > 0:
            sum["tradeprice_ynygrowth"] = str("%0.2f" % (
            (float(sum["tradeprice"]) - float(sum["tradepriceold"])) * 100.0 / float(sum["tradepriceold"]))) + "%"
        else:
...
def writeDataToSheet1(wb, rlist, sumDict):
    ...
    titles = [[("（%s月）日销售报表" % month, 0, 1, 15)],
              [("数据日期：", 0, 1, 2), (yesterday, 2, 1, 1), ("月时间进度：", 5, 1, 1),
               ("{progress}%".format(progress=progress), 6, 1, 1), ("单位：万元", 21, 1, 1)],
              [("区域", 0, 3, 1), ("店号", 1, 3, 1), ("店名", 2, 3, 1), ("开业时间", 3, 3, 1), ("类型", 4, 3, 1),
               ("当日销售", 5, 1, 15), ("月累计销售额", 20, 1, 8),
               ("月毛利", 28, 1, 10), ("月日均来客数", 38, 1, 3), ("月客单价", 41, 1, 3)],
              [("销售", 5, 1, 4), ("来客数", 9, 1, 3), ("客单价", 12, 1, 3), ("毛利", 15, 1, 5), ("实际", 20, 2, 1),
               ("预算", 21, 2, 1), ("差异", 22, 2, 1),
               ("达成率", 23, 2, 1), ("全月预算", 24, 2, 1), ("月预算进度", 25, 2, 1), ("去年销售", 26, 2, 1), ("同比增长", 27, 2, 1),
               ("实际", 28, 2, 1), ("预算", 29, 2, 1), ("差异", 30, 2, 1), ("达成率", 31, 2, 1), ("全月预算", 32, 2, 1),
               ("月预算进度", 33, 2, 1), ("去年同期", 34, 2, 1),
               ("同比增长", 35, 2, 1), ("毛利率", 36, 2, 1), ("去年同期毛利率", 37, 2, 1), ("月累计来客数", 38, 2, 1), ("去年", 39, 2, 1),
               ("同比增长", 40, 2, 1),
               ("月累计客单价", 41, 2, 1), ("去年", 42, 2, 1), ("同比增长", 43, 2, 1)],
              [("实际", 5, 1, 1), ("预算", 6, 1, 1), ("差异", 7, 1, 1), ("达成率", 8, 1, 1), ("当日", 9, 1, 1), ("去年", 10, 1, 1),
               ("同比增长", 11, 1, 1)
                  , ("当日", 12, 1, 1), ("去年", 13, 1, 1), ("同比增长", 14, 1, 1), ("毛利额", 15, 1, 1), ("预算", 16, 1, 1),
               ("差异", 17, 1, 1), ("达成率", 18, 1, 1)
                  , ("毛利率", 19, 1, 1)]
              ]

    keylist = ['region', 'shopid', 'shopname', 'opentime', 'type', 'day_salevalue', 'day_salevalueesti',
               'day_sale_difference', 'day_accomratio',
               'day_tradenumber', 'day_tradenumberold', 'day_tradenumber_ynygrowth', 'day_tradeprice',
               'day_tradepriceold', 'day_tradeprice_ynygrowth',
               'day_salegain', 'day_salegainesti', 'day_salegain_difference', 'day_salegain_accomratio',
               'day_grossmargin', 'month_salevalue'
        , 'month_salevalueesti', 'month_sale_difference', 'month_accomratio', 'month_sale_estimate',
               'month_complet_progress', 'month_salevalueold',
               'month_sale_ynygrowth', 'month_salegain', 'month_salegainesti', 'month_salegain_difference',
               'month_salegain_accomratio', 'month_salegain_estimate',
               'month_salegain_complet_progress', 'month_salegainold', 'month_salegain_ynygrowth',
               'month_salegain_grossmargin', 'month_salegain_grossmarginold',
               'month_tradenumber', 'month_tradenumberold', 'month_tradenumber_ynygrowth', 'month_tradeprice',
               'month_tradepriceold', 'month_tradeprice_ynygrowth']
...
def writeDataToSheet2(wb, erlist, esumDict):
    ...
    titles = [[("（%s月）日销售报表" % month, 0, 1, 15)],
              [("区域", 0, 2, 1), ("店号", 1, 2, 1), ("店名", 2, 2, 1), ("开业时间", 3, 2, 1), ("类型", 4, 2, 1), ("累计实际", 5, 2, 1),
               ("累计预算", 6, 2, 1)],
              []]

    titles2 = [[("（%s月）日毛利报表" % month, 0, 1, 15)],
               [("区域", 0, 2, 1), ("店号", 1, 2, 1), ("店名", 2, 2, 1), ("开业时间", 3, 2, 1), ("类型", 4, 2, 1),
                ("月累计毛利", 5, 2, 1), ("月累计预算", 6, 2, 1)],
               []]
...
def writeDataToSheet3(wb, yearlist, yearSumDict):
    ...
    titles = [[("年累计销售报表", 0, 1, 15)],
              [("数据日期：", 0, 1, 2), (yesterday, 2, 1, 1), ("月时间进度：", 5, 1, 1),
               ("{progress}%".format(progress=progress), 6, 1, 1), ("单位：万元", 8, 1, 1)],
              [("区域", 0, 2, 1), ("店号", 1, 2, 1), ("店名", 2, 2, 1), ("开业时间", 3, 2, 1), ("类型", 4, 2, 1),
               ("年累计销售额", 5, 1, 8), ("年累计毛利", 13, 1, 9),
               ("年日均来客数", 22, 1, 3), ("年日均客单价", 25, 1, 3)],
              [("实际", 5, 2, 1), ("累计预算", 6, 2, 1), ("差异", 7, 2, 1), ("达成率", 8, 2, 1), ("全年预算", 9, 2, 1),
               ("年预算进度", 10, 2, 1),
               ("去年", 11, 2, 1), ("同比增长", 12, 2, 1), ("实际", 13, 2, 1), ("累计预算", 14, 2, 1), ("差异", 15, 2, 1),
               ("达成率", 16, 2, 1), ("全年预算", 17, 2, 1),
               ("年预算进度", 18, 2, 1), ("去年", 19, 2, 1), ("同比增长", 20, 2, 1), ("毛利率", 21, 2, 1), ("实际", 22, 2, 1),
               ("去年", 23, 2, 1), ("同比增长", 24, 2, 1),
               ("年累计", 25, 2, 1), ("去年", 26, 2, 1), ("同比增长", 27, 2, 1)],
              ]

    keylist = ['region', 'shopid', 'shopname', 'opentime', 'type', 'salevalue', 'salevalueesti', 'sale_difference',
               'sale_accomratio',
               'y_salevalue', 'sale_complet_progress', 'salevalueold', 'sale_ynygrowth', 'salegain', 'salegainesti',
               'salegain_difference', 'salegain_accomratio', 'y_salegain', 'salegain_complet_progress', 'salegainold',
               'salegain_ynygrowth'
        , 'salegain_grossmargin', 'tradenumber', 'tradenumberold', 'tradenumber_ynygrowth', 'tradeprice',
               'tradepriceold',
